...
Deno.serve(async (req) => {
  ...
  // Find existing auth user by email (paginate through all users).
  // If not found, create one via admin API so the same email can register again.
  ...
  try {
    ...
    while (page < 20 && !userId) {
      ...
      const found = list.find((u: any) => (u.email || "").toLowerCase() === emailLower);
  ...
  if (!userId) {
    // No auth user yet — create one so the profile can be linked.
    const tempPassword = typeof password === "string" && password.length >= 8 ? password : crypto.randomUUID() + "Aa1!";
    const { data: created, error: createErr } = await adminClient.auth.admin.createUser({
      email: emailLower,
      password: tempPassword,
      email_confirm: true,
      user_metadata: { user_type: uType, full_name: nameUpper },
    });
    if (createErr || !created?.user) return errResp(500, createErr?.message || "Failed to create auth user");
