...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!pendingSecret) return json({ error: "No TOTP setup in progress" }, 400);

    const valid = await verifyTOTP(pendingSecret, String(token));
    if (!valid)
      return json({ error: "Incorrect code. Check your authenticator app and try again." }, 400);
