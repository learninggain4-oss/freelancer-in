  ...
  return (
    <div className="min-h-screen pb-10" style={{ background: T.bg, color: T.text }}>
      ...
      <div className="px-4 sm:px-6 mb-5 grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: "Total Earned",    value: `₹${totalEarned.toLocaleString("en-IN")}`,     color: "#4ade80", icon: TrendingUp },
          { label: "Withdrawn",       value: `₹${totalWithdrawn.toLocaleString("en-IN")}`,  color: "#60a5fa", icon: ArrowDownToLine },
          { label: "Available",       value: `₹${Number(pendingBal).toLocaleString("en-IN")}`, color: "#6366f1", icon: Wallet },
          { label: "On Hold",         value: `₹${Number(holdBal).toLocaleString("en-IN")}`, color: "#fbbf24", icon: Clock },
        ].map(s => {
          const Icon = s.icon;
          return (
          ...
          {isLoading ? (
            ...
            displayTxns.map((t: any, i: number) => (
              <div key={t.id ?? i} className="flex items-center justify-between px-4 py-3.5 transition-all" style={{ borderBottom: i < displayTxns.length - 1 ? `1px solid ${T.border}` : "none" }}>
                ...
                <span className="text-sm font-black" style={{ color: t.type === "credit" ? "#4ade80" : "#f87171" }}>
