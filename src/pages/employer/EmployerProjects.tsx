  ...
  return (
    <div className="space-y-6 p-4 pb-24 min-h-screen" style={{ backgroundColor: T.bg, color: T.text }}>
      ...
      <Tabs defaultValue="projects" className="w-full">
        <TabsList
          className="w-full p-1 h-14 rounded-2xl border mb-6"
          style={{ background: T.nav, borderColor: T.border }}
        >
          <TabsTrigger
            value="projects"
            className="flex-1 rounded-xl text-xs font-bold data-[state=active]:shadow-lg"
            style={{ color: T.text }}
          >
            My Jobs
          </TabsTrigger>
        ...
        <TabsContent value="projects" className="mt-0 space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
          {loadingProjects ? (
            ...
            myProjects.map((p: any) => (
              <Card
                key={p.id}
                className="border-0 shadow-xl group overflow-hidden"
                style={{ background: T.card, border: `1px solid ${T.border}`, backdropFilter: "blur(12px)" }}
              >
                <CardContent className="space-y-4 p-5">
                  <div className="flex items-start justify-between">
                    <div className="space-y-1">
                      <h3 className="text-lg font-bold tracking-tight" style={{ color: T.text }}>
                        {p.name}
                      </h3>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    >
                      ...
                      <span className="flex items-center gap-1 text-sm font-bold" style={{ color: T.text }}>
                        <Calendar className="h-3.5 w-3.5 text-indigo-400" />
                        {p.end_date ?? "No deadline"}
                      </span>
                    </div>
                  </div>
                        ...
                        <AlertDialog>
                          <AlertDialogTrigger asChild>
                            <Button
                              size="sm"
                              variant="outline"
                              className="h-10 w-10 rounded-xl border-rose-500/30 text-rose-500 hover:bg-rose-500/10"
                            >
                          ...
                          <AlertDialogContent className="bg-[#070714] border-white/10 rounded-3xl">
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription className="text-slate-400">
                                This action cannot be undone. All related applications will be lost.
                              </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
            ...
            <div
              className="flex flex-col items-center justify-center py-20 rounded-3xl border-2 border-dashed border-white/5"
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="text-base font-bold tracking-tight" style={{ color: T.text }}>
                          {r.freelancer?.full_name ?? "Freelancer"}
                        </h3>
                        <p className="text-[10px] font-bold uppercase tracking-widest" style={{ color: T.sub }}>
                          {r.freelancer?.user_code} • {r.freelancer?.work_experience ?? "Fresh"}
                        </p>
                      </div>
                    </div>
                    <Badge
        ...
        <TabsContent
          value="submissions"
          className="mt-0 space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500"
        >
          {loadingSubs ? (
              ...
              >
                <CardContent className="flex items-center justify-between p-5">
                  <div className="space-y-1">
                    ...
                    <p className="text-[10px] font-bold uppercase tracking-widest" style={{ color: T.sub }}>
                      Submitted by <span className="text-indigo-400">{s.freelancer?.full_name}</span> •{" "}
