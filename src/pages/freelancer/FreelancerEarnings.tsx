...
const TH = {
  black:  { bg: "#070714",  card: "rgba(255,255,255,.05)", border: "rgba(255,255,255,.08)", text: "#e2e8f0", sub: "#94a3b8", input: "rgba(255,255,255,.07)", tip: { background: "rgba(13,13,36,.95)", border: "1px solid rgba(255,255,255,.1)",  color: "#e2e8f0" }, axis: "rgba(255,255,255,.3)" },
  ...
  wb:     { bg: "#f0f4ff",  card: "#ffffff",               border: "rgba(0,0,0,.08)",       text: "#1e293b", sub: "#64748b", input: "#f8fafc",               tip: { background: "#fff",                border: "1px solid rgba(0,0,0,.1)",       color: "#1e293b" }, axis: "#9ca3af" },
  warm:   { bg: "#fef6e4",  card: "#fffdf7",               border: "rgba(180,83,9,.1)",     text: "#1c1a17", sub: "#78716c", input: "#fffdf7",               tip: { background: "#fffdf7",             border: "1px solid rgba(180,83,9,.15)",   color: "#1c1a17" }, axis: "#a8a29e" },
  forest: { bg: "#f1faf4",  card: "#ffffff",               border: "rgba(21,128,61,.1)",    text: "#0f2d18", sub: "#4b7c5d", input: "#ffffff",               tip: { background: "#ffffff",             border: "1px solid rgba(21,128,61,.15)",  color: "#0f2d18" }, axis: "#4b7c5d" },
  ocean:  { bg: "#f0f9ff",  card: "#ffffff",               border: "rgba(14,165,233,.1)",   text: "#0c4a6e", sub: "#4b83a3", input: "#ffffff",               tip: { background: "#ffffff",             border: "1px solid rgba(14,165,233,.15)", color: "#0c4a6e" }, axis: "#4b83a3" },
};
...
export default function EmployeeEarnings() {
  ...
  const displayTxns = txns;

  const totalEarned   = txns.filter((t: any) => t.type === "credit").reduce((s: number, t: any) => s + Number(t.amount), 0);
  const totalWithdrawn= txns.filter((t: any) => t.type === "debit").reduce((s: number, t: any) => s + Number(t.amount), 0);
  ...
  const monthlyChart = (() => {
    ...
    txns.filter((t: any) => t.type === "credit").forEach((t: any) => {
      const key = new Date(t.created_at).toLocaleString("en-IN", { month: "short" });
      if (key in map) map[key] += Number(t.amount);
    });
    return Object.entries(map).map(([month, earned]) => ({ month, earned }));
  })();
    ...
    <div className="min-h-screen pb-24" style={{ background: T.bg, color: T.text }}>
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-4 mb-4">
              <div className="flex h-14 w-14 items-center justify-center rounded-2xl shadow-xl" style={{ background: "rgba(255,255,255,.2)", backdropFilter: "blur(12px)" }}>
                <TrendingUp className="h-7 w-7" style={{ color: "white" }} />
              </div>
              <div>
                <h1 className="text-2xl font-black tracking-tight" style={{ color: "white" }}>Earnings & Invoice</h1>
                <p className="text-xs font-bold uppercase tracking-widest" style={{ color: "rgba(255,255,255,.75)" }}>Your income overview</p>
              </div>
            </div>
            <div className="mb-4">
              <p className="text-[10px] font-bold uppercase tracking-widest mb-0.5" style={{ color: "rgba(255,255,255,.65)" }}>Total Earned</p>
              <p className="text-4xl font-black" style={{ color: "white" }}>₹{totalEarned.toLocaleString("en-IN")}</p>
            </div>
            <div className="flex gap-3 overflow-x-auto pb-1" style={{ scrollbarWidth: "none" }}>
              {[
                { label: "Withdrawn", value: `₹${totalWithdrawn.toLocaleString("en-IN")}` },
                { label: "Available", value: `₹${Number(pendingBal).toLocaleString("en-IN")}` },
                { label: "On Hold",   value: `₹${Number(holdBal).toLocaleString("en-IN")}` },
              ].map(s => (
                <div key={s.label} className="shrink-0 rounded-2xl px-4 py-2.5" style={{ background: "rgba(255,255,255,.15)", backdropFilter: "blur(8px)" }}>
                  <p className="text-base font-black" style={{ color: "white" }}>{s.value}</p>
      ...
      <div className="px-4 sm:px-6">
        <div className="flex items-center gap-2.5 mb-3">
          ...
          <h3 className="text-sm font-black" style={{ color: T.text }}>Transaction History</h3>
        </div>
        <div className="rounded-2xl overflow-hidden" style={card}>
          {isLoading ? (
            ...
            <div className="py-12 text-center px-4">
              <IndianRupee className="h-10 w-10 mx-auto mb-3 opacity-25" style={{ color: T.sub }} />
              <p className="text-sm font-semibold" style={{ color: T.text }}>No transactions yet</p>
              <p className="text-[11px] mt-1" style={{ color: T.sub }}>Your transaction history will appear here</p>
            </div>
          ) : (
