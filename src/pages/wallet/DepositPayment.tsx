...
export default function DepositPayment() {
  ...
  const legacyPaymentDeadline = req?.payment_deadline
    || (req?.status === "payment_shared" && req?.updated_at
      ? new Date(new Date(req.updated_at).getTime() + PAYMENT_TIMEOUT_MIN * 60 * 1000).toISOString()
      : (req?.status === "payment_shared" && req?.created_at
        ? new Date(new Date(req.created_at).getTime() + PAYMENT_TIMEOUT_MIN * 60 * 1000).toISOString()
        : null));
  const paymentSecs = useCountdown(legacyPaymentDeadline);
  ...
  const submitUtr = async () => {
    // Validation
    if (!utrInput.trim()) {
      toast.error("Please enter a UTR/Transaction ID"); return;
    }
    if (utrInput.trim().length < 4) {
      toast.error("Enter a valid UTR number (min 4 characters)"); return;
    }
  ...
  const uploadProof = async (file: File | null) => {
    ...
    try {
      ...
      const path = `deposit-proofs/${requestId}/${Date.now()}.${safeExt}`;
      const { error: upErr } = await supabase.storage.from("payment-attachments").upload(path, file, {
        contentType: file.type || "image/jpeg",
      });
      if (upErr) throw upErr;
      const { data } = supabase.storage.from("payment-attachments").getPublicUrl(path);
      setProofMeta({ path, url: data.publicUrl, name: file.name });
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 480, margin: "0 auto", padding: "20px 16px 0" }}>
          ...
          <div style={{ textAlign: "center", padding: "32px 24px", animation: "fadeUp .5s ease" }}>
            ...
            <p style={{ color: "rgba(255,255,255,.45)", fontSize: 13, lineHeight: 1.6, marginBottom: 24 }}>
              {firstWindowSecs !== null && firstWindowSecs > 0
                ? "Your payment window is active. Please wait while we reserve your slot and prepare admin payment details."
                : "Countdown completed. Please wait a little more while admin shares payment details."}
              <br /><br />
        ...
        {req.status === "payment_shared" && pd && Object.keys(pd).length > 0 && (
          <div style={{ animation: "fadeUp .4s ease" }}>
            ...
            {/* UTR Form - Show if payment time remaining and not yet submitted */}
            {(paymentSecs ?? 1) > 0 && req.status === "payment_shared" && (
