...
import { CheckCircle, XCircle, BadgeCheck, Pencil, Save, EyeOff, ChevronDown, ChevronUp } from "lucide-react";
...
const AdminWithdrawals = () => {
  ...
  const fetchWithdrawals = async () => {
    ...
    let query = supabase
      .from("withdrawals")
      .select("id, amount, method, status, requested_at, review_notes, employee_id, upi_id, bank_account_number, bank_ifsc_code, bank_holder_name, is_cleared")
  ...
  const handleAction = async () => {
    ...
    if (res.error || res.data?.error) { toast.error(res.data?.error || "Failed to update withdrawal"); }
  ...
  const handleClear = async (id: string) => {
    ...
    else { toast.success("Withdrawal cleared"); fetchWithdrawals(); }
  };

  const handleRestore = async (id: string) => {
    const { error } = await supabase.from("withdrawals").update({ is_cleared: false }).eq("id", id);
    if (error) toast.error(error.message);
    else { toast.success("Withdrawal restored"); fetchWithdrawals(); }
  };
  ...
  const handleSaveEdit = async () => {
    ...
    const { error } = await supabase.from("withdrawals").update({
      ...
      review_notes: editForm.review_notes || null,
      method: editForm.method,
    }).eq("id", expandedEdit);
    if (error) toast.error(error.message);
    else { toast.success("Withdrawal updated"); setExpandedEdit(null); fetchWithdrawals(); }
  };
  ...
  const WithdrawalTable = ({ items }: { items: Withdrawal[] }) => (
    <div className="overflow-x-auto rounded-lg border">
      <Table>
        ...
        <TableBody>
          {items.length === 0 ? (
            <TableRow><TableCell colSpan={8} className="py-8 text-center text-muted-foreground">No withdrawals found</TableCell></TableRow>
          ) : (
            items.map((w) => (
              <>
                <TableRow key={w.id} className={w.is_cleared ? "opacity-60" : ""}>
                  <TableCell>
                    <div className="flex items-center gap-1.5">
                      ...
                      {w.bankVerified ? (
                        ...
                        <span className="shrink-0 rounded-full bg-warning/15 px-1.5 py-0.5 text-[10px] font-medium text-warning">Unverified</span>
                      )}
                    </div>
                  </TableCell>
                  ...
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-1">
                      <Button size="icon" variant="ghost" onClick={() => startEdit(w)} title="Edit">
                      ...
                      {w.is_cleared ? (
                        <Button size="sm" variant="ghost" onClick={() => handleRestore(w.id)} className="text-xs">Restore</Button>
                      ) : (
                        <AlertDialog>
                          <AlertDialogTrigger asChild>
                            <Button size="icon" variant="ghost" className="text-destructive" title="Clear">
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>This will soft-delete the withdrawal record. It can be restored later.</AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
                      ...
                      {w.status === "rejected" && w.review_notes && (
                        <p className="text-xs text-destructive max-w-[200px] truncate" title={w.review_notes}>{w.review_notes}</p>
                      )}
                    </div>
                  </TableCell>
                </TableRow>
                    ...
                    <TableCell colSpan={8} className="bg-muted/30 p-4">
                      <div className="grid gap-3 sm:grid-cols-4">
                        ...
                        <div className="space-y-1">
                          ...
                          <Input value={editForm.method} onChange={(e) => setEditForm((f: any) => ({ ...f, method: e.target.value }))} />
                        </div>
                        <div className="space-y-1">
                        ...
                        <div className="space-y-1">
                          ...
                          <Input value={editForm.review_notes} onChange={(e) => setEditForm((f: any) => ({ ...f, review_notes: e.target.value }))} />
                        </div>
                      </div>
                      <Button className="mt-3" onClick={handleSaveEdit}>
      ...
      <Dialog open={!!selected && !!actionType} onOpenChange={(open) => { if (!open) { setSelected(null); setActionType(null); setNotes(""); } }}>
        <DialogContent className="max-w-md">
          ...
          <div>
            ...
            <Textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder={actionType === "reject" ? "Enter rejection reason..." : "Add notes..."} />
          </div>
          <DialogFooter>
