...
const AdminMobileVerifySchedule = () => {
  ...
  const queryKey = ["admin-mobile-verify-schedule"];
  ...
  const { data: rows = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await (supabase as any)
        .from("profiles")
        .select("id, full_name, user_code, email, user_type, mobile_number, verify_window_start, verify_window_end")
  ...
  useEffect(() => {
    if (editing) {
      setStart(fmt(editing.verify_window_start));
      setEnd(fmt(editing.verify_window_end));
    }
  }, [editing]);
  ...
  const save = async () => {
    if (!editing) return;
    if (!start || !end) { toast.error("Start and End time required"); return; }
    if (start === end) { toast.error("Start and End time cannot be the same"); return; }
    setSaving(true);
    const { error } = await (supabase as any)
      .from("profiles")
      .update({ verify_window_start: start, verify_window_end: end, verify_window_changed_at: null })
      .eq("id", editing.id);
    setSaving(false);
  ...
  const clearWindow = async (id: string) => {
    const { error } = await (supabase as any)
      .from("profiles")
      .update({ verify_window_start: null, verify_window_end: null, verify_window_changed_at: null })
      .eq("id", id);
    if (error) { toast.error(error.message); return; }
  ...
  const configured = rows.filter((r) => r.verify_window_start && r.verify_window_end).length;

  return (
    <div className="space-y-6 p-4 md:p-6">
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="icon" onClick={() => navigate("/admin/mobile-verifications")}>
          <ArrowLeft className="h-5 w-5" />
        </Button>
        <div>
          <h1 className="text-2xl font-bold tracking-tight" style={{ color: T.text }}>
            Mobile Verify Setup Time
          </h1>
          <p className="mt-1 text-sm" style={{ color: T.sub }}>
            Manage daily Mobile Verify time windows for freelancers & employers.
          </p>
        </div>
      </div>
            ...
            <table className="min-w-[900px] w-full text-sm">
              ...
              <tbody>
                {isLoading ? (
                  ...
                  filtered.map((r) => {
                    const isConfigured = r.verify_window_start && r.verify_window_end;
                    return (
                      <tr key={r.id} className="border-b last:border-0" style={{ borderColor: T.border }}>
                        ...
                        <td className="px-3 py-3 font-mono" style={{ color: T.text }}>{fmt(r.verify_window_start) || "-"}</td>
                        <td className="px-3 py-3 font-mono" style={{ color: T.text }}>{fmt(r.verify_window_end) || "-"}</td>
                        <td className="px-3 py-3">
                          {isConfigured ? (
      ...
      <Dialog open={!!editing} onOpenChange={(o) => !o && setEditing(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Set Mobile Verify Time Window</DialogTitle>
          </DialogHeader>
...
export default AdminMobileVerifySchedule;
