...
import { IndianRupee, CheckCircle, XCircle, Loader2, MessageSquare } from "lucide-react";
...
const AdminRecoveryRequests = () => {
  ...
  const { data: requests = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select(`
          *,
          project:project_id(id, name, amount, validation_fees, status, assigned_employee_id),
          employee:employee_id(id, full_name, user_code, hold_balance, available_balance)
        `)
  ...
  const handleReject = async (requestId: string) => {
    ...
    try {
      const { error } = await supabase
        ...
        .update({
          status: "rejected",
          admin_notes: adminNotes[requestId] || "Rejected by admin",
  ...
  const openSupportChat = async (request: any) => {
    // Find or create the support chat room
    ...
    if (existing) {
      navigate(`/admin/recovery-chat/${request.project_id}?room=${existing.id}`);
    } else {
      ...
      const { data: newRoom, error } = await supabase
        ...
        .single();
      if (error) {
        toast.error("Failed to create support chat room");
        return;
      }
              ...
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription>
                      Employee: {req.employee?.full_name?.[0] || "Unknown"} ({req.employee?.user_code?.[0]})
                    </CardDescription>
                  </div>
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          ...
                          <AlertDialogFooter>
                            ...
                            <AlertDialogAction
                              onClick={() => handleRelease(req.id, req.project_id, req.employee_id)}
                            >
                              Confirm Release
                            </AlertDialogAction>
                          </AlertDialogFooter>
