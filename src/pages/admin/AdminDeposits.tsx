...
import {
  ...
  Eye, Share2, ChevronDown, Smartphone, Building2, X, AlertCircle, Edit3,
} from "lucide-react";
...
function parsePaymentDetails(raw: any): Record<string, any> {
  if (!raw) return {};
...
function ShareModal({ req, onClose, onShared }: ShareModalProps) {
  ...
  const share = async () => {
    ...
    const { data, error } = await supabase.functions.invoke("wallet-operations", {
      body: {
        action: "admin_share_deposit_details",
    ...
    if (error || data?.error) {
      ...
      if (msg.toLowerCase().includes("unknown action")) {
        // Backward-compatible fallback for older wallet-operations deployments.
        const { error: fallbackError } = await supabase.from("deposit_requests").update({
          admin_payment_details: details,
          payment_deadline: deadline,
        ...
        if (fallbackError) {
          ...
          // Legacy fallback chain for older deposit_requests schemas.
          const tryLegacyUpdate = async () => {
            const base = { status: "payment_shared" };

            // Try with payment_details + payment_deadline first.
            let r = await supabase.from("deposit_requests").update({
              ...base,
            ...
            const msg1 = r.error.message?.toLowerCase() || "";
            // If payment_deadline column is missing, retry without it.
            if (msg1.includes("payment_deadline")) {
              r = await supabase.from("deposit_requests").update({
                ...base,
                payment_details: details,
              }).eq("id", req.id);
              if (!r.error) return null;
          ...
          const legacyError = await tryLegacyUpdate();
          if (legacyError) {
            toast.error(legacyError.message || fallbackMsg || "Failed to share details");
            setSharing(false);
            return;
          }
        }
      } else {
        toast.error(msg || "Failed to share details");
        setSharing(false);
        return;
      }
    }
...
function ReviewModal({ req, onClose, onDone }: ReviewModalProps) {
  ...
  const action = async (approve: boolean) => {
    ...
    try {
      ...
      if (approve) {
        const { error: balErr } = await (supabase.rpc as any)("increment_balance", {
  ...
  const userPd = parsePaymentDetails(req.payment_details);
  const proofUrl = userPd?.user_payment_screenshot_url as string | undefined;
  const proofName = (userPd?.user_payment_screenshot_name as string | undefined) || "Payment Screenshot";
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 1000, background: "rgba(0,0,0,.5)", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div style={{ background: T.card, borderRadius: 20, width: "100%", maxWidth: 480, boxShadow: "0 24px 64px rgba(0,0,0,.2)" }}>
        ...
        <div style={{ padding: "20px 24px" }}>
            ...
            <div style={{ background: "#ede9fe", borderRadius: 12, padding: "14px 16px", marginBottom: 16, border: "1px solid #ddd6fe" }}>
              ...
              {proofUrl && (
                <p style={{ fontSize: 12, margin: "8px 0 0" }}>
                  <a href={proofUrl} target="_blank" rel="noreferrer" style={{ color: "#4f46e5", fontWeight: 700, textDecoration: "underline" }}>
                    View {proofName}
                  </a>
                </p>
              )}
          ...
          {(userPd.otp_sent_at || userPd.otp_value) && (
            <div style={{ background: userPd.otp_value ? "#dcfce7" : "#f0fdf4", borderRadius: 12, padding: "14px 16px", marginBottom: 16, border: `1px solid ${userPd.otp_value ? "#86efac" : "#bbf7d0"}` }}>
              <p style={{ fontSize: 10, fontWeight: 800, color: "#16a34a", marginBottom: 8, textTransform: "uppercase" }}>OTP Verification</p>
              {userPd.otp_sent_at && (
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
                  <CheckCircle2 size={14} color="#16a34a" />
                  <span style={{ fontSize: 12, color: "#15803d", fontWeight: 600 }}>
                    OTP Requested — {new Date(userPd.otp_sent_at).toLocaleString("en-IN")}
                  </span>
                </div>
              )}
              {userPd.otp_value ? (
                <div>
                  <p style={{ fontSize: 10, fontWeight: 700, color: "#15803d", margin: "0 0 4px", textTransform: "uppercase" }}>OTP Submitted by User</p>
                  <p style={{ fontSize: 28, fontWeight: 900, fontFamily: "monospace", color: "#14532d", margin: 0, letterSpacing: ".2em" }}>{userPd.otp_value}</p>
                  {userPd.otp_submitted_at && (
                    <p style={{ fontSize: 11, color: "#16a34a", margin: "4px 0 0" }}>at {new Date(userPd.otp_submitted_at).toLocaleString("en-IN")}</p>
                  )}
                </div>
              ) : (
                <p style={{ fontSize: 12, color: "#16a34a", margin: 0, fontStyle: "italic" }}>Waiting for user to enter OTP…</p>
              )}
            </div>
          )}
...
export default function AdminDeposits() {
  ...
  const updateStatusMutation = useMutation({
    mutationFn: async ({ id, newStatus }: { id: string; newStatus: string }) => {
      ...
      if (error || data?.error) throw new Error(data?.error || error?.message || "Failed to update status");
      return data;
    },
    onSuccess: () => {
  ...
  const filtered = requests.filter(r => {
    ...
    const details = parsePaymentDetails(r.payment_details);
    const referenceId = (r.order_id || r.id || "").toLowerCase();
    const methodPhone = (details.phone_number || details.phone || "").toLowerCase();
    return name.toLowerCase().includes(s) || referenceId.includes(s) || r.utr_number?.toLowerCase().includes(s) || methodPhone.includes(s);
  });
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24, flexWrap: "wrap", gap: 12 }}>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <div style={{ position: "relative" }}>
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search reference ID, name, UTR…"
        ...
        <div style={{ background: T.card, borderRadius: 16, border: `1px solid ${T.bdr}`, overflow: "hidden", boxShadow: "0 4px 24px rgba(0,0,0,.04)" }}>
          {loading ? (
            ...
            <div>
              ...
              {/* Table header */}
              <div style={{ display: "grid", gridTemplateColumns: "1.1fr 1fr .8fr .8fr 1fr .9fr .9fr .9fr .8fr 160px",
                ...
                return (
                  <div key={r.id} style={{ borderBottom: `1px solid ${T.bdr}` }}>
                    <div style={{ display: "grid", gridTemplateColumns: "1.1fr 1fr .8fr .8fr 1fr .9fr .9fr .9fr .8fr 160px",
                      ...
                      <div>
                        <p style={{ fontSize: 12, fontWeight: 800, fontFamily: "monospace", color: T.text, margin: 0 }}>{r.order_id || r.id}</p>
                      </div>

                      <div>
                        <p style={{ fontSize: 11, color: T.sub, margin: 0 }}>{name}</p>
                        {r.utr_number && (
                        ...
                        {details.otp_sent_at && (
                          <div style={{ display: "flex", alignItems: "center", gap: 4, marginTop: 2 }}>
                            ...
                            <span style={{ fontSize: 10, fontWeight: 700, color: "#16a34a" }}>OTP Requested</span>
                          </div>
                        )}
                        {details.otp_value && (
                          <p style={{ fontSize: 11, fontFamily: "monospace", fontWeight: 800, color: "#2563eb", margin: "2px 0 0", letterSpacing: ".1em" }}>
                            OTP: {details.otp_value}
                          </p>
                        )}
                      </div>
                      ...
                      <div>
                        <span style={{ fontSize: 11, fontWeight: 600, color: T.sub }}>
                          {r.profiles?.user_type === "employee" ? "Freelancer" : r.profiles?.user_type === "employer" ? "Employer" : "—"}
                        </span>
                      </div>
                      ...
                      <div>
                        <span style={{ fontSize: 11, color: T.sub, margin: 0 }}>
                        ...
                        {editingStatusId === r.id ? (
                          <select
                            ...
                            onChange={(e) => {
                              ...
                              console.log("Status change:", r.id, "->", newStatus);
                              updateStatusMutation.mutate({ id: r.id, newStatus });
                            }}
                            disabled={updateStatusMutation.isPending}
                            className="text-xs font-bold rounded-lg px-2 py-1"
                            style={{ background: T.muted, color: T.text, border: `1px solid ${T.bdr}`, cursor: "pointer" }}
                            autoFocus
                          ...
                          <div style={{ display: "flex", alignItems: "center", gap: 2, cursor: "pointer" }} onClick={(e) => { e.stopPropagation(); setEditingStatusId(r.id); }} title="Click to edit status">
                            <StatusBadge status={r.status} />
                            <Edit3 size={14} color={T.accent} style={{ opacity: 0.7 }} />
                          </div>
                        )}
                      </div>
                    ...
                    {expanded && (
                      <div style={{ padding: "16px 24px 20px", background: "rgba(99,102,241,.025)", borderTop: `1px solid ${T.bdr}`,
                        ...
                        {(details.otp_sent_at || details.otp_value) && (
                          <div>
                            <p style={{ fontSize: 10, fontWeight: 800, color: T.sub, textTransform: "uppercase", marginBottom: 6 }}>OTP Verification</p>
                            {details.otp_sent_at && (
                              <div style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 4 }}>
                                <CheckCircle2 size={12} color="#16a34a" />
                                <span style={{ fontSize: 11, color: "#16a34a", fontWeight: 600 }}>OTP Requested</span>
                              </div>
                            )}
                            {details.otp_value && (
                              <p style={{ fontSize: 18, fontWeight: 900, fontFamily: "monospace", color: "#1d4ed8", margin: "2px 0 0", letterSpacing: ".15em" }}>
                                OTP: {details.otp_value}
                              </p>
                            )}
                          </div>
