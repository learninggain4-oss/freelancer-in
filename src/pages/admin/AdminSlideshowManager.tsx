...
const TH = {
  black: { card: "rgba(255,255,255,.05)", border: "rgba(255,255,255,.08)", text: "#e2e8f0", sub: "#94a3b8", input: "rgba(255,255,255,.07)", bg: "#0f0f23" },
  white: { card: "#ffffff", border: "rgba(0,0,0,.08)", text: "#1e293b", sub: "#64748b", input: "#f8fafc", bg: "#f8fafc" },
    ...
    try {
      ...
      if (upErr) {
        if (upErr.message?.includes("Bucket not found") || upErr.message?.includes("bucket")) {
          toast.error("Storage bucket not set up. Please create a public bucket named 'slideshow-banners' in Supabase Storage.");
        } else {
  ...
  return (
    <div style={{ padding: "24px 16px", maxWidth: 900, margin: "0 auto" }}>
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ color: T.sub, fontSize: 13, marginTop: 4 }}>Manage image banners shown above the FlexPay Wallet card</p>
      </div>
      ...
      <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 16, padding: 20, marginBottom: 24 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
          <div>
            ...
            <select
              ...
              style={{ width: "100%", background: T.input, border: `1px solid ${T.border}`, borderRadius: 8, color: T.text, padding: "8px 10px", fontSize: 13 }}
            >
          ...
          <div>
            ...
            <input
              ...
              style={{ width: "100%", background: T.input, border: `1px solid ${T.border}`, borderRadius: 8, color: T.text, padding: "8px 10px", fontSize: 13, boxSizing: "border-box" }}
            />
          </div>
        </div>
        ...
        <button
          ...
          disabled={uploading}
          style={{
            background: `linear-gradient(135deg,${A1},#8b5cf6)`, border: "none", borderRadius: 10,
            padding: "10px 20px", cursor: uploading ? "not-allowed" : "pointer", color: "#fff",
        ...
        <p style={{ fontSize: 11, color: T.sub, marginTop: 8 }}>Recommended: 1200×375px, JPG/PNG, max 5MB. Aspect ratio 16:5 works best.</p>
      </div>

      {loading ? (
        <div style={{ textAlign: "center", padding: 40, color: T.sub }}>Loading...</div>
      ) : slides.length === 0 ? (
        <div style={{ textAlign: "center", padding: 40, color: T.sub, background: T.card, border: `1px solid ${T.border}`, borderRadius: 16 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {[...slides].sort((a, b) => a.sort_order - b.sort_order).map((slide, idx) => (
            <div key={slide.id} style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 14, overflow: "hidden", display: "flex", gap: 0 }}>
              <div style={{ width: 140, flexShrink: 0 }}>
                <img src={slide.image_url} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", minHeight: 80 }} />
              </div>
              <div style={{ flex: 1, padding: "12px 16px", display: "flex", flexDirection: "column", gap: 8, justifyContent: "center" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                  <span style={{ fontSize: 12, fontWeight: 700, color: slide.active ? "#4ade80" : "#94a3b8", background: slide.active ? "rgba(74,222,128,.12)" : "rgba(148,163,184,.12)", borderRadius: 6, padding: "2px 9px", border: `1px solid ${slide.active ? "rgba(74,222,128,.3)" : "rgba(148,163,184,.3)"}` }}>
                    {slide.active ? "Live" : "Hidden"}
                  </span>
                  <select
                    value={slide.target}
                    onChange={e => updateTarget(slide.id, e.target.value as Target)}
                    style={{ background: T.input, border: `1px solid ${T.border}`, borderRadius: 6, color: T.text, padding: "3px 8px", fontSize: 12 }}
                  >
                    <option value="all">Everyone</option>
                ...
                {slide.link_url && <div style={{ fontSize: 11, color: T.sub, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>🔗 {slide.link_url}</div>}
              </div>
              <div style={{ display: "flex", flexDirection: "column", gap: 6, padding: "12px", justifyContent: "center", flexShrink: 0 }}>
                <button onClick={() => move(slide.id, "up")} disabled={idx === 0} title="Move Up"
                  style={{ background: "none", border: `1px solid ${T.border}`, borderRadius: 6, padding: "5px", cursor: "pointer", color: T.sub, display: "flex", opacity: idx === 0 ? 0.3 : 1 }}>
                  <ArrowUp size={13} />
                </button>
                <button onClick={() => move(slide.id, "down")} disabled={idx === slides.length - 1} title="Move Down"
                  style={{ background: "none", border: `1px solid ${T.border}`, borderRadius: 6, padding: "5px", cursor: "pointer", color: T.sub, display: "flex", opacity: idx === slides.length - 1 ? 0.3 : 1 }}>
                  <ArrowDown size={13} />
                </button>
                <button onClick={() => toggle(slide.id)} title={slide.active ? "Hide" : "Show"}
                  style={{ background: "none", border: `1px solid ${T.border}`, borderRadius: 6, padding: "5px", cursor: "pointer", color: slide.active ? "#fbbf24" : "#4ade80", display: "flex" }}>
                  {slide.active ? <EyeOff size={13} /> : <Eye size={13} />}
                </button>
                <button onClick={() => remove(slide.id, slide.image_url)} title="Delete"
                  style={{ background: "rgba(248,113,113,.1)", border: "1px solid rgba(248,113,113,.3)", borderRadius: 6, padding: "5px", cursor: "pointer", color: "#f87171", display: "flex" }}>
                  <Trash2 size={13} />
                </button>
              </div>
            </div>
