...
import { Clock, ArrowLeft, Plus, Pencil, Trash2, Loader2, CheckCircle2, XCircle, Search, User } from "lucide-react";
...
const AdminMobileVerifySetupTime = () => {
  ...
  const { data: slots = [], isLoading } = useQuery({
    queryKey: ["mobile-verify-time-slots"],
    queryFn: async () => {
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      if (!form.profile_id) throw new Error("Please select a user");
      ...
      if (editId) {
        const { error } = await supabase.from("mobile_verify_time_slots").update({
        ...
        const { error } = await supabase.from("mobile_verify_time_slots").upsert({
          profile_id: form.profile_id, start_time: form.start_time, end_time: form.end_time, status: form.status,
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["mobile-verify-time-slots"] });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from("mobile_verify_time_slots").delete().eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => { toast.success("Deleted"); qc.invalidateQueries({ queryKey: ["mobile-verify-time-slots"] }); setDeleteId(null); },
    onError: (e: any) => toast.error(e.message),
  });

  const toggleStatus = useMutation({
    mutationFn: async ({ id, status }: { id: string; status: string }) => {
      const { error } = await supabase.from("mobile_verify_time_slots").update({ status }).eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["mobile-verify-time-slots"] }),
    onError: (e: any) => toast.error(e.message),
  });
    ...
    <div style={{ padding:"20px 16px", maxWidth:900, margin:"0 auto" }}>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:20, flexWrap:"wrap", justifyContent:"space-between" }}>
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          <button onClick={() => navigate("/admin/mobile-verification")} style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:8, padding:"6px 10px", cursor:"pointer", color:T.sub }}>
            <ArrowLeft size={16} />
          </button>
          ...
          <div>
            <h1 style={{ fontWeight:800, fontSize:20, color:T.text, margin:0 }}>Verify Setup Time</h1>
            <p style={{ color:T.sub, fontSize:12, marginTop:2 }}>Set the mobile verification time window for each user</p>
          </div>
        </div>
        <button onClick={openAdd} style={{ display:"flex", alignItems:"center", gap:6, padding:"8px 16px", background:A1, border:"none", borderRadius:10, color:"#fff", fontSize:13, fontWeight:700, cursor:"pointer" }}>
      ...
      {dialogOpen && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.6)", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", padding:16 }}>
          <div style={{ background:themeKey==="black"?"#0f0f23":"#fff", border:`1px solid ${T.border}`, borderRadius:16, padding:28, maxWidth:420, width:"100%" }}>
            <h2 style={{ fontWeight:800, fontSize:17, color:T.text, marginBottom:18 }}>{editId ? "Edit" : "Add"} Verify Time Slot</h2>
...
export default AdminMobileVerifySetupTime;
