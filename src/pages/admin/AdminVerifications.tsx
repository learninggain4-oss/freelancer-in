...
const AdminVerifications = () => {
  ...
  // Status changing state for selected verification
  ...
  const { data: verifications = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select(
          "id, profile_id, aadhaar_number, name_on_aadhaar, dob_on_aadhaar, address_on_aadhaar, front_image_path, back_image_path, status, rejection_reason, created_at, verified_at, profiles!aadhaar_verifications_profile_id_fkey(full_name, user_code, email, user_type, date_of_birth, mobile_number)",
        )
  ...
  const actionMutation = useMutation({
    ...
    onSuccess: (_, vars) => {
      toast.success(`Verification status updated to ${vars.status}`);
    ...
    mutationFn: async ({ ids, status, reason }: { ids: string[]; status: string; reason?: string }) => {
      ...
      for (const id of ids) {
        ...
        if (status === "verified") {
          await supabase
            .from("profiles")
            .update({ full_name: [v.name_on_aadhaar] })
            .eq("id", v.profile_id);
        }
      }
    },
  ...
  const openDetail = async (v: Verification) => {
    ...
    setCustomStatus(v.status);
    setRejectReason(v.rejection_reason || "");
    setIsEditing(false);
    setEditForm({
      name_on_aadhaar: v.name_on_aadhaar,
      aadhaar_number: v.aadhaar_number,
      dob_on_aadhaar: v.dob_on_aadhaar,
      address_on_aadhaar: v.address_on_aadhaar,
    });
  ...
  const exportCSV = () => {
    const rows = [
      ["Name on Aadhaar", "Aadhaar (last 4)", "User Name", "User Code", "Email", "Status", "Submitted", "Verified At"],
    ];
    filtered.forEach((v) =>
      rows.push([
        v.name_on_aadhaar || "—",
    ...
    <div className="space-y-6 p-4 min-h-screen" style={{ backgroundColor: T.bg, color: T.text }}>
      ...
      >
        <div>
          ...
          <p style={{ color: T.sub }}>Review and manage verification submissions and control custom statuses</p>
        </div>
        <div className="flex items-center gap-3">
          <button
            onClick={exportCSV}
            className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold"
            style={{ background: "rgba(99,102,241,.15)", border: "1px solid rgba(99,102,241,.3)", color: "#6366f1" }}
          >
          ...
          <div className="flex items-center gap-4 bg-black/20 p-3 rounded-xl border border-white/5">
            <div className="flex items-center gap-2">
              ...
              <Label htmlFor="show-cleared-aadhaar" className="text-xs cursor-pointer" style={{ color: T.sub }}>
                Show cleared
              </Label>
            </div>
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          {
            label: "Under Process",
            count: verifications.filter((v) => v.status === "under_process").length,
            color: "#60a5fa",
          ...
          <button
            ...
            style={{
              padding: "14px 16px",
              borderRadius: 12,
              background: tab === s.tab ? `${s.color}14` : T.card,
              border: `1px solid ${tab === s.tab ? s.color + "50" : T.border}`,
              cursor: "pointer",
              textAlign: "left",
              transition: "all .2s",
              width: "100%",
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6, color: s.color }}>
              {s.icon}
              <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: 0.4 }}>{s.label.toUpperCase()}</span>
            </div>
      ...
      <div className="relative group">
        <Search
          className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 transition-colors group-focus-within:text-indigo-500"
          style={{ color: T.sub }}
        />
        <Input
          placeholder="Search by name, identifier, email, or code..."
      ...
      >
        ...
        <TabsContent value={tab} className="mt-6 space-y-4">
          ...
          {actionableFiltered.length > 0 && (
            <div
              className="flex flex-wrap items-center gap-4 p-4 rounded-xl border backdrop-blur-md transition-all"
          ...
          {isLoading ? (
            ...
            <Card className="border-dashed py-12" style={{ backgroundColor: "transparent", borderColor: T.border }}>
              <CardContent className="flex flex-col items-center justify-center text-center">
                ...
                <p style={{ color: T.sub }}>No records found for this status tab</p>
              </CardContent>
            </Card>
          ) : (
      ...
      >
        <DialogContent
          className="max-h-[90vh] overflow-y-auto sm:max-w-2xl p-0 border-none shadow-2xl"
          style={{ backgroundColor: T.bg }}
        >
          <div
            className="sticky top-0 z-10 p-6 flex items-center justify-between border-b backdrop-blur-xl"
            style={{ backgroundColor: `${T.card}`, borderColor: T.border }}
          >
          ...
          {selectedVerification && (
            <div className="p-6 space-y-6">
              ...
              <div className="flex justify-end gap-3">
                <Button
                  ...
                  onClick={() => {
                    ...
                    if (!isEditing)
                      setEditForm({
                        name_on_aadhaar: selectedVerification.name_on_aadhaar,
                        aadhaar_number: selectedVerification.aadhaar_number,
                        dob_on_aadhaar: selectedVerification.dob_on_aadhaar,
                        address_on_aadhaar: selectedVerification.address_on_aadhaar,
                      });
                  }}
                >
                  ...
                  <AlertDialogContent className="border-white/10" style={{ backgroundColor: T.bg }}>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription style={{ color: T.sub }}>
                        This will permanently delete this verification record and its uploaded images. This action
                        cannot be undone.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                      ...
                      <AlertDialogAction
                        ...
                        onClick={() => deleteMutation.mutate(selectedVerification)}
                      >
              ...
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <Card className="border-none shadow-lg overflow-hidden" style={{ backgroundColor: T.card }}>
                  <CardHeader className="pb-4 border-b border-white/5">
                    <CardTitle className="text-sm font-semibold uppercase tracking-wider" style={{ color: T.sub }}>
                      Profile Information
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="pt-6 space-y-4">
                    {[
                      ...
                      <div key={idx} className="flex flex-col gap-1">
                        <span className="text-[10px] font-medium uppercase opacity-40" style={{ color: T.text }}>
                          {item.label}
                        </span>
                        {item.badge ? (
                          <Badge
                ...
                {/* Verification info - View or Edit mode */}
                <Card className="border-none shadow-lg overflow-hidden" style={{ backgroundColor: T.card }}>
                  <CardHeader className="pb-4 border-b border-white/5">
                    <CardTitle className="text-sm font-semibold uppercase tracking-wider" style={{ color: T.sub }}>
                      Document Details
                    ...
                    {isEditing ? (
                      <div className="space-y-4">
                        <div className="space-y-2">
                          ...
                          <Input
                            ...
                            style={{ backgroundColor: T.input, color: T.text }}
                            value={editForm.name_on_aadhaar}
                            onChange={(e) => setEditForm((f) => ({ ...f, name_on_aadhaar: e.target.value }))}
                          />
                        </div>
                        <div className="space-y-2">
                          <Label className="text-xs" style={{ color: T.sub }}>
                          ...
                          <Input
                            ...
                            onChange={(e) => setEditForm((f) => ({ ...f, aadhaar_number: e.target.value }))}
                          />
                        </div>
                        <div className="space-y-2">
                          <Label className="text-xs" style={{ color: T.sub }}>
                          ...
                          <Input
                            ...
                            type="date"
                            style={{ backgroundColor: T.input, color: T.text }}
                            value={editForm.dob_on_aadhaar}
                            onChange={(e) => setEditForm((f) => ({ ...f, dob_on_aadhaar: e.target.value }))}
                          />
                        </div>
                        <div className="space-y-2">
                          <Label className="text-xs" style={{ color: T.sub }}>
                          ...
                          <Textarea
                            ...
                            style={{ backgroundColor: T.input, color: T.text }}
                            value={editForm.address_on_aadhaar}
                            onChange={(e) => setEditForm((f) => ({ ...f, address_on_aadhaar: e.target.value }))}
                          />
                        </div>
                        <Button
                          className="w-full bg-indigo-600 hover:bg-indigo-700 text-white shadow-lg shadow-indigo-500/20"
                          disabled={editMutation.isPending}
                          onClick={() => editMutation.mutate({ id: selectedVerification.id, data: editForm })}
                        >
                      ...
                      <div className="space-y-5">
                        <div className="flex flex-col gap-1">
                          ...
                          <span className="text-sm font-bold tracking-widest" style={{ color: T.text }}>
                            {selectedVerification.aadhaar_number}
                          </span>
                        </div>
                        <div className="flex flex-col gap-1">
                          <span className="text-[10px] font-medium uppercase opacity-40" style={{ color: T.text }}>
                          ...
                          <span className="text-sm font-medium" style={{ color: T.text }}>
                            {selectedVerification.name_on_aadhaar}
                          </span>
                        </div>
                        <div className="flex flex-col gap-1">
                          <span className="text-[10px] font-medium uppercase opacity-40" style={{ color: T.text }}>
                          ...
                          <span className="text-sm font-medium" style={{ color: T.text }}>
                            {selectedVerification.dob_on_aadhaar}
                          </span>
                        </div>
                        <div className="flex flex-col gap-1">
                          <span className="text-[10px] font-medium uppercase opacity-40" style={{ color: T.text }}>
                          ...
                          <span className="text-sm leading-relaxed" style={{ color: T.text }}>
                            {selectedVerification.address_on_aadhaar}
                          </span>
                        </div>
                        ...
                        {selectedVerification.profiles?.full_name?.[0] && (
                          ...
                          >
                            {selectedVerification.name_on_aadhaar.toLowerCase().trim() ===
                            selectedVerification.profiles.full_name[0].toLowerCase().trim() ? (
                              <>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                {[
                  ...
                  <div key={i} className="space-y-3">
                    <p className="text-[10px] font-bold uppercase tracking-widest opacity-50" style={{ color: T.text }}>
                      {img.label}
                    </p>
              ...
              {/* Live Status Modification Controls */}
              <div
                className="space-y-4 p-5 rounded-2xl border"
                style={{ backgroundColor: "rgba(255,255,255,0.02)", borderColor: T.border }}
              >
                <div className="flex flex-col gap-2">
                  <Label className="text-xs font-bold uppercase tracking-widest opacity-60" style={{ color: T.text }}>
                    Change Verification Status Directly
                  </Label>
                  <div className="flex flex-wrap gap-2 items-center mt-1">
                    <select
                      value={customStatus}
                      onChange={(e) => setCustomStatus(e.target.value)}
                      className="h-10 px-3 rounded-lg border text-sm focus:outline-none"
                    ...
                    <Button
                      ...
                      disabled={actionMutation.isPending || (customStatus === "rejected" && !rejectReason.trim())}
                      onClick={() =>
                        actionMutation.mutate({
                          id: selectedVerification.id,
                          status: customStatus,
                          reason: customStatus === "rejected" ? rejectReason : undefined,
                          profileId: selectedVerification.profile_id,
                          nameOnAadhaar: selectedVerification.name_on_aadhaar,
                        })
                      }
                    >
                      {actionMutation.isPending ? (
                        ...
                        <>
                          <RefreshCw className="mr-1.5 h-3.5 w-3.5" /> Apply Status
                        </>
                      )}
                    </Button>
                  </div>
                ...
                {customStatus === "rejected" && (
                  <div className="space-y-2 pt-2 animate-in fade-in slide-in-from-top-2 duration-200">
                    ...
                    <Textarea
                      placeholder="Please add comments regarding why this document was rejected..."
                      className="border-red-500/20 min-h-[80px] focus-visible:ring-red-500/30"
                      style={{ backgroundColor: "rgba(0,0,0,0.15)", color: T.text }}
                      value={rejectReason}
