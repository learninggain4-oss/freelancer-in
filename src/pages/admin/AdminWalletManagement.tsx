        ...
        <>
          ...
          <Tabs defaultValue="transactions">
            ...
            <TabsContent value="audit">
              {(() => {
                const totalALPages = Math.max(1, Math.ceil(auditLogs.length / AL_PAGE_SIZE));
                const safeALPage = Math.min(alPage, totalALPages);
                const paginatedAL = auditLogs.slice((safeALPage - 1) * AL_PAGE_SIZE, safeALPage * AL_PAGE_SIZE);

                const actionLabels: Record<string, { label: string; color: string }> = {
                  wallet_add: { label: "Add Money", color: "bg-accent/15 text-accent border-accent/30" },
                  wallet_deduct: { label: "Deduct", color: "bg-destructive/15 text-destructive border-destructive/30" },
                  wallet_hold: { label: "Hold", color: "bg-warning/15 text-warning border-warning/30" },
                  wallet_transfer: { label: "Transfer", color: "bg-primary/15 text-primary border-primary/30" },
                  edit_transaction: { label: "Edit Tx", color: "bg-muted text-muted-foreground border-muted-foreground/30" },
                  delete_transaction: { label: "Delete Tx", color: "bg-destructive/15 text-destructive border-destructive/30" },
                  edit_withdrawal: { label: "Edit Withdrawal", color: "bg-muted text-muted-foreground border-muted-foreground/30" },
                  delete_withdrawal: { label: "Delete Withdrawal", color: "bg-destructive/15 text-destructive border-destructive/30" },
                };
                ...
                return (
                  <div className="space-y-3">
                    <div className="overflow-x-auto rounded-lg border">
                      <Table>
                        ...
                        <TableBody>
                          {loadingAL ? (
                            <TableRow><TableCell colSpan={5}><Skeleton className="h-8 w-full" /></TableCell></TableRow>
                          ) : paginatedAL.length === 0 ? (
                            <TableRow><TableCell colSpan={5} className="py-6 text-center text-muted-foreground">No audit logs</TableCell></TableRow>
                          ) : paginatedAL.map((log) => {
                            const info = actionLabels[log.action] || { label: log.action, color: "" };
                            const details = log.details || {};
                            let detailStr = "";
                            if (details.amount) detailStr += `₹${Number(details.amount).toLocaleString("en-IN")}`;
                            if (details.description) detailStr += detailStr ? ` — ${details.description}` : details.description;
                            if (details.to_name) detailStr += ` → ${details.to_name}`;
                            if (details.adjust_balance !== undefined) detailStr += details.adjust_balance ? " (balance adjusted)" : " (log only)";
                            return (
                              <TableRow key={log.id}>
                                <TableCell>
                                  <Badge variant="outline" className={info.color}>{info.label}</Badge>
                                </TableCell>
                                <TableCell className="text-sm">{(log.admin_profile as any)?.full_name?.[0] || "—"}</TableCell>
                                <TableCell className="text-sm">{log.target_profile_name || "—"}</TableCell>
                                <TableCell className="max-w-[250px] truncate text-xs text-muted-foreground">{detailStr || "—"}</TableCell>
                    ...
                    {auditLogs.length > AL_PAGE_SIZE && (
                      <div className="flex items-center justify-between">
                        <p className="text-sm text-muted-foreground">
                          Showing {(safeALPage - 1) * AL_PAGE_SIZE + 1}–{Math.min(safeALPage * AL_PAGE_SIZE, auditLogs.length)} of {auditLogs.length}
                        </p>
                        <div className="flex items-center gap-1">
                          <Button size="icon" variant="outline" className="h-8 w-8" disabled={safeALPage <= 1} onClick={() => setAlPage(safeALPage - 1)}>
                            <ChevronLeft className="h-4 w-4" />
                          </Button>
                          {Array.from({ length: totalALPages }, (_, i) => i + 1)
                            .filter((p) => p === 1 || p === totalALPages || Math.abs(p - safeALPage) <= 1)
                            .reduce<(number | "ellipsis")[]>((acc, p, idx, arr) => {
                              if (idx > 0 && p - (arr[idx - 1] as number) > 1) acc.push("ellipsis");
                              acc.push(p);
                              return acc;
                            ...
                            .map((p, i) =>
                              p === "ellipsis" ? (
                                ...
                                <Button key={p} size="icon" variant={p === safeALPage ? "default" : "outline"} className="h-8 w-8 text-xs" onClick={() => setAlPage(p)}>
                                  {p}
                                </Button>
                              )
                            )}
                          <Button size="icon" variant="outline" className="h-8 w-8" disabled={safeALPage >= totalALPages} onClick={() => setAlPage(safeALPage + 1)}>
                            <ChevronRight className="h-4 w-4" />
                          </Button>
                        </div>
                      </div>
