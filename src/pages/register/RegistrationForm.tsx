...
const RegistrationForm = ({ userType }: RegistrationFormProps) => {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      const { data, error } = await supabase.from("profiles").select("id").ilike("username", v).maybeSingle();
      if (error) {
        setUsernameStatus("idle");
        return;
      }
  ...
  const validateCurrentStep = async (): Promise<boolean> => {
    ...
    if (type === "personal" && usernameStatus === "taken") {
      setArrayErrors(["Username is already taken. Please choose another."]);
      return false;
    }
    if (type === "work") {
      const errors: string[] = [];
      workExperiences.forEach((w, i) => {
        const e = validateWorkExperience(w);
        if (e) errors.push(`Experience ${i + 1}: ${e}`);
      });
      if (errors.length) {
    ...
    if (type === "emergency") {
      if (!emergencyContacts.length) {
        setArrayErrors(["At least one emergency contact is required"]);
        return false;
      }
      const errors: string[] = [];
      emergencyContacts.forEach((c, i) => {
        const e = validateEmergencyContact(c);
        if (e) errors.push(`Contact ${i + 1}: ${e}`);
      });
      if (errors.length) {
        setArrayErrors(errors);
        return false;
    ...
    if (type === "services" && categories.length > 0 && services.length > 0) {
      ...
      services.forEach((s, i) => {
        const e = validateService(s);
        if (e) errors.push(`Service ${i + 1}: ${e}`);
      });
      if (errors.length) {
        setArrayErrors(errors);
        return false;
  ...
  const onSubmit = async (data: RegistrationFormData) => {
    const lastValid = await validateCurrentStep();
    if (!lastValid) return;
    setSubmitting(true);
    try {
      let geoData: any = {};
      ...
      const insertRelatedData = async (profileId: string, supabaseClient: typeof supabase) => {
        try {
          await supabaseClient.from("registration_metadata" as any).insert([
            {
        ...
        for (const w of workExperiences.filter((w) => w.company_name.trim())) {
          ...
          try {
            await supabaseClient.from("work_experiences").insert({
              profile_id: profileId,
              company_name: w.company_name,
              company_type: w.company_type,
              work_description: w.work_description || null,
        ...
        for (const c of emergencyContacts.filter((c) => c.contact_name.trim())) {
          try {
            await supabaseClient.from("employee_emergency_contacts").insert({
              profile_id: profileId,
            ...
            if (svcData && s.skill_ids.length > 0) {
              try {
                await supabaseClient
                  .from("employee_skill_selections")
                  .insert(
  ...
  const updateWorkExp = (i: number, f: keyof WorkExperienceEntry, v: any) =>
    setWorkExperiences((p) => p.map((w, j) => (j === i ? { ...w, [f]: v } : w)));
  const updateContact = (i: number, f: keyof EmergencyContactEntry, v: string) =>
    setEmergencyContacts((p) => p.map((c, j) => (j === i ? { ...c, [f]: v } : c)));
  const updateService = (i: number, f: keyof ServiceEntry, v: any) =>
    setServices((p) => p.map((s, j) => (j === i ? { ...s, [f]: v } : s)));
        ...
        <div style={{ width: "100%", maxWidth: 440, animation: "fadeUp .5s ease both" }}>
          ...
          <div style={{ textAlign: "center", marginBottom: 28 }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 8, textDecoration: "none" }}>
              ...
              <span style={{ fontWeight: 800, fontSize: 16, color: "white" }}>
                Freelancer<span style={{ color: "#6366f1" }}>.</span>in
              </span>
            </div>
          </div>
          ...
          {showSuccess ? (
            ...
            >
              ...
              <h2 style={{ fontSize: 26, fontWeight: 800, color: "#ffffff", margin: "0 0 8px" }}>
              ...
              >
                <p style={{ color: "rgba(255,255,255,0.6)", fontSize: 13, margin: 0 }}>
                  Redirecting to Login in <strong style={{ color: "#818cf8", fontSize: 16 }}>{redirectSec}</strong>{" "}
                  seconds…
                </p>
              </div>
              <button
              ...
              >
                      ...
                      background:
                        i % 3 === 0
                          ...
                          : i % 3 === 1
                            ...
                            : "rgba(34,197,94,0.45)",
                      animation: `particleFloat ${3.2 + i * 0.65}s ease-in-out infinite ${i * 0.45}s`,
                      pointerEvents: "none",
                    }}
                  />
                ))}
                ...
                <div
                  ...
                  style={{
                    ...
                    animation: "cardBorderFlash 1.1s ease forwards",
                    pointerEvents: "none",
                    zIndex: 2,
                  }}
                />
                ...
                <div style={{ textAlign: "center", marginBottom: 24, position: "relative", zIndex: 1 }}>
                  ...
                  <p style={{ color: "rgba(255,255,255,0.45)", fontSize: 13, margin: 0 }}>
                    Please stay on this page. Do not close the browser.
                  </p>
                </div>
                      ...
                      >
                        ...
                        <p
                          ...
                          style={{
                            ...
                            position: "relative",
                            animation: "digitFlip 0.25s ease both",
                          }}
                        >
                ...
                <div style={{ textAlign: "center", marginBottom: 14 }}>
                  ...
                  >
                    Step {Math.min(activeIdx + 1, REG_STEPS.length)}{" "}
                    <span style={{ color: "rgba(255,255,255,0.3)" }}>of</span> {REG_STEPS.length}
                  </span>
                </div>
                      ...
                      >
                          ...
                          >
                            {done ? (
                              <CheckCircle2
                                size={15}
                                color="#22c55e"
                                style={{ animation: "checkBounce 0.4s cubic-bezier(.36,1.56,.64,1) both" }}
                              />
                            ) : active ? (
                        ...
                        <p
                          ...
                          style={{
                            ...
                            color: done ? "#86efac" : active ? "#c7d2fe" : "rgba(255,255,255,0.28)",
                            margin: 0,
                            lineHeight: 1.3,
                ...
                <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                  ...
                  <div>
                    ...
                    <p style={{ color: "rgba(255,255,255,0.5)", fontSize: 12, margin: 0, lineHeight: 1.6 }}>
                      Creating <strong style={{ color: "rgba(255,255,255,0.75)" }}>multiple accounts</strong> is
                      strictly prohibited on Freelancer.in. Users found with duplicate accounts will be{" "}
                      <strong style={{ color: "#fca5a5" }}>permanently banned</strong> and any pending earnings or coins
                      will be <strong style={{ color: "#fca5a5" }}>forfeited</strong>. One person, one account — always.
                    </p>
                  </div>
                </div>
              </div>
      ...
      <div style={{ position: "fixed", inset: 0, pointerEvents: "none", zIndex: 0 }}>
        ...
        <div
          style={{
            ...
            animation: "orbGlow 8s ease-in-out infinite 2s",
          }}
        />
        <div
          style={{
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 580, margin: "0 auto", padding: "32px 20px 80px" }}>
                ...
                >
                  <div
                    style={{
                      ...
                      alignItems: "center",
                      justifyContent: "center",
                      border: `2px solid ${isActive ? stepColor : isDone ? "#22c55e" : "rgba(255,255,255,.1)"}`,
                      background: isActive
                        ? `${stepColor}22`
                        : isDone
                          ? "rgba(34,197,94,.12)"
                  ...
                  >
                    {isDone ? (
                      ...
                      <Icon size={14} color={isActive ? stepColor : "rgba(255,255,255,.3)"} />
                    )}
                  </div>
                  <span
                    style={{
                      ...
                      color: isActive ? stepColor : isDone ? "#22c55e" : "rgba(255,255,255,.3)",
                      textAlign: "center",
                      lineHeight: 1.2,
                    }}
                  >
                  ...
                  {i < stepConfig.length - 1 && (
                    <div
                      style={{
                        position: "absolute",
                        height: 2,
                        background: isDone ? "rgba(34,197,94,.4)" : "rgba(255,255,255,.08)",
                      }}
                    />
                  )}
                </div>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription style={{ color: "rgba(255,255,255,.3)", fontSize: 11 }}>
                          Name will be stored in uppercase
                        </FormDescription>
                          ...
                          <div style={{ position: "relative" }}>
                            ...
                            >
                              {usernameStatus === "checking" && (
                                <Loader2 size={16} color="rgba(255,255,255,.5)" className="animate-spin" />
                              )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input type="date" {...field} className="reg-input" style={inp} />
                          </FormControl>
                          <FormMessage className="text-red-400 text-xs" />
                        </FormItem>
                      )}
              ...
              {stepType === "contact" && (
                <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input placeholder="10-digit" maxLength={10} {...field} className="reg-input" style={inp} />
                          </FormControl>
                          <FormMessage className="text-red-400 text-xs" />
                        </FormItem>
                      )}
                  ...
                  >
                    ...
                    />
                    <p style={{ color: "rgba(255,255,255,.3)", fontSize: 11, marginTop: 6 }}>
                      Got a referral code? Enter it to earn bonus coins!
                    </p>
                  </div>
                </div>
              )}
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                    <div>
                      ...
                      <Select
                        value={employerBiz.business_type}
                        onValueChange={(v) => setEmployerBiz((p) => ({ ...p, business_type: v }))}
                      >
                        <SelectTrigger style={inp}>
                    ...
                    <div>
                      ...
                      <Select
                        value={employerBiz.industry_sector}
                        onValueChange={(v) => setEmployerBiz((p) => ({ ...p, industry_sector: v }))}
                      >
                        <SelectTrigger style={inp}>
                  ...
                  <div>
                    ...
                    >
                      <FileText size={13} color="rgba(255,255,255,.4)" /> GST Number{" "}
                      <span style={{ color: "rgba(255,255,255,.3)", fontSize: 11, fontWeight: 400, marginLeft: 4 }}>
                        (optional)
                      </span>
                    </label>
                    <Input
                  ...
                  <div>
                    ...
                    <Textarea
                      ...
                      className="reg-input"
                      style={{ ...inp, padding: "10px 12px", minHeight: 88, resize: "vertical" }}
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <p style={{ color: "rgba(255,255,255,.25)", fontSize: 11, marginTop: 5 }}>
                      Per project / assignment range you usually hire for
                    </p>
                  </div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                    ...
                    <div>
                      ...
                      <Select
                        value={employerBiz.state}
                        onValueChange={(v) => setEmployerBiz((p) => ({ ...p, state: v }))}
                      >
                        <SelectTrigger style={inp}>
              ...
              {stepType === "work" && (
                <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Textarea
                            placeholder="Institutions, degrees, certifications…"
                            rows={2}
                            {...field}
                            className="reg-input"
                            style={{ ...inp, padding: "10px 12px", minHeight: 72, resize: "vertical" }}
                          />
                        </FormControl>
                        <FormMessage className="text-red-400 text-xs" />
                      </FormItem>
                  ...
                  {workExperiences.map((w, i) => (
                    <div key={i} className="reg-card">
                      ...
                      >
                        ...
                        <button
                          type="button"
                          onClick={() => setWorkExperiences((p) => p.filter((_, j) => j !== i))}
                          style={{
                            width: 28,
                            height: 28,
                            borderRadius: 8,
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                        <Input
                          ...
                          onChange={(e) => updateWorkExp(i, "company_name", e.target.value)}
                          className="reg-input"
                          style={inp}
                        />
                        ...
                        <Textarea
                          ...
                          rows={2}
                          value={w.work_description}
                          onChange={(e) => updateWorkExp(i, "work_description", e.target.value)}
                          className="reg-input"
                          style={{ ...inp, padding: "10px 12px", minHeight: 64, resize: "vertical" }}
                        />
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          <Checkbox
                            ...
                            onCheckedChange={(c) => updateWorkExp(i, "is_current", !!c)}
                            id={`current-${i}`}
                          />
                          <label
                            htmlFor={`current-${i}`}
                            style={{ color: "rgba(255,255,255,.6)", fontSize: 12, cursor: "pointer" }}
                          >
                        ...
                        >
                          ...
                          <Input
                            ...
                            onChange={(e) => updateWorkExp(i, "certificate_file", e.target.files?.[0] || null)}
                          />
                        </div>
                      </div>
                    </div>
                  ...
                  <button
                    ...
                    onClick={() => setWorkExperiences((p) => [...p, emptyWorkExp()])}
                  >
              ...
              {stepType === "emergency" && (
                <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                  ...
                  >
                    ...
                    <p style={{ color: "rgba(255,255,255,.45)", fontSize: 12, lineHeight: 1.6 }}>
                      Add at least one emergency contact. This info is kept private and used only in critical
                      situations.
                    </p>
                  </div>
                    ...
                    <div key={i} className="reg-card">
                      ...
                      >
                        ...
                        {emergencyContacts.length > 1 && (
                          <button
                            type="button"
                            onClick={() => setEmergencyContacts((p) => p.filter((_, j) => j !== i))}
                            style={{
                              width: 28,
                              height: 28,
                              borderRadius: 8,
                  ...
                  <button
                    type="button"
                    className="reg-add-btn"
                    style={{ borderColor: "rgba(244,63,94,.35)", color: "#f43f5e" }}
                    onClick={() => setEmergencyContacts((p) => [...p, emptyContact()])}
                  >
                  ...
                  {categories.length === 0 ? (
                    ...
                    >
                      ...
                      <p style={{ color: "rgba(255,255,255,.35)", fontSize: 13 }}>
                        No service categories configured yet. You can update your services later from your profile.
                      </p>
                    </div>
                  ) : (
                    <>
                      <p style={{ color: "rgba(255,255,255,.4)", fontSize: 12 }}>
                        Add the services you offer. Select a category, set your rates, and pick your skills.
                      </p>
                      {services.map((s, i) => {
                            ...
                            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                              ...
                              <div>
                                <Input
                                  ...
                                  onChange={(e) => updateService(i, "service_title", e.target.value)}
                                  className="reg-input"
                                  style={inp}
                                />
                                <p style={{ color: "rgba(255,255,255,.3)", fontSize: 11, marginTop: 4 }}>
                                  e.g., Full Stack Development, Graphic Design
                                </p>
                              </div>
                                      ...
                                      return (
                                        <span
                                          ...
                                          style={{
                                            ...
                                            color: selected ? A1 : "rgba(255,255,255,.5)",
                                          }}
                                        >
                                          {sk.name}
                                        </span>
                              ...
                              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                                <div>
                                  ...
                                  <Input
                                    ...
                                    placeholder="0"
                                    value={s.hourly_rate}
                                    onChange={(e) => updateService(i, "hourly_rate", e.target.value)}
                                    className="reg-input"
                                    style={inp}
                                  />
                                </div>
                                <div>
                                  ...
                                  <Input
                                    ...
                                    min="0"
                                    placeholder="0"
                                    value={s.minimum_budget}
                                    onChange={(e) => updateService(i, "minimum_budget", e.target.value)}
                                    className="reg-input"
                                    style={inp}
                                  />
                                </div>
                      ...
                      <button
                        type="button"
                        className="reg-add-btn"
                        style={{ borderColor: "rgba(245,158,11,.35)", color: "#f59e0b" }}
        ...
        <div
          style={{
            ...
            gap: 12,
            animation: "fadeInUp .5s ease both .15s",
          }}
        >
          {/* Terms (last step) */}
          {step === stepConfig.length - 1 && (
            ...
            >
              ...
              <label
                htmlFor="reg-terms"
                style={{ fontSize: 12, color: "rgba(255,255,255,.5)", lineHeight: 1.5, cursor: "pointer" }}
              >
                I agree to the{" "}
                <Link to="/legal/terms-of-service" target="_blank" style={{ color: A1 }}>
                  Terms of Service
                </Link>{" "}
                and{" "}
                <Link to="/legal/privacy-policy" target="_blank" style={{ color: A1 }}>
                  Privacy Policy
                </Link>
              </label>
            </div>
                  ...
                  <form onSubmit={form.handleSubmit(onSubmit)} style={{ flex: 1 }}>
                    <button
                      ...
                      style={{
                        ...
                        borderRadius: 14,
                        background:
                          submitting || !agreedToTerms ? "rgba(99,102,241,.3)" : `linear-gradient(135deg,${A1},${A2})`,
                        color: "white",
                        fontWeight: 700,
                        fontSize: 14,
                        border: "none",
                        ...
                        justifyContent: "center",
                        gap: 6,
                        boxShadow: submitting || !agreedToTerms ? "none" : "0 8px 24px rgba(99,102,241,.35)",
                      }}
                    >
                      {submitting ? (
          ...
          <p style={{ textAlign: "center", fontSize: 12, color: "rgba(255,255,255,.35)" }}>
            ...
            <Link to="/login" style={{ color: A1, fontWeight: 700, textDecoration: "none" }}>
              Login here
            </Link>
          </p>
        </div>
