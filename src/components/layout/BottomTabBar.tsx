    ...
    >
      ...
      >
        ...
        >
          <NavLink to={`${base}/projects`} onClick={() => setActiveSubMenu(null)} style={submenuLinkStyle}>
            <div style={{ background: activePill, padding: "10px", borderRadius: "12px" }}>
              <Briefcase size={24} color={activeC} />
            </div>
            <span style={{ fontSize: 12, fontWeight: 600, color: labelActive }}>Jobs</span>
          </NavLink>
          <NavLink to={`${base}/requests`} onClick={() => setActiveSubMenu(null)} style={submenuLinkStyle}>
            <div style={{ background: activePill, padding: "10px", borderRadius: "12px" }}>
              <FileText size={24} color={activeC} />
            </div>
            <span style={{ fontSize: 12, fontWeight: 600, color: labelActive }}>Requests</span>
          </NavLink>
        </div>
        ...
        >
          <NavLink to={`${base}/wallet`} onClick={() => setActiveSubMenu(null)} style={submenuLinkStyle}>
            <div style={{ background: activePill, padding: "10px", borderRadius: "12px" }}>
              <CreditCard size={22} color={activeC} />
            </div>
            <span style={{ fontSize: 11.5, fontWeight: 600, color: labelActive }}>Account</span>
          </NavLink>
          <NavLink to={`${base}/my-qr`} onClick={() => setActiveSubMenu(null)} style={submenuLinkStyle}>
            <div style={{ background: activePill, padding: "10px", borderRadius: "12px" }}>
              <QrCode size={22} color={activeC} />
            </div>
            <span style={{ fontSize: 11.5, fontWeight: 600, color: labelActive }}>My QR</span>
          </NavLink>
          <NavLink to={`${base}/scan-qr`} onClick={() => setActiveSubMenu(null)} style={submenuLinkStyle}>
            <div style={{ background: activePill, padding: "10px", borderRadius: "12px" }}>
              <Scan size={22} color={activeC} />
            </div>
            <span style={{ fontSize: 11.5, fontWeight: 600, color: labelActive }}>Scan QR</span>
          </NavLink>
          <NavLink to={`${base}/attendance`} onClick={() => setActiveSubMenu(null)} style={submenuLinkStyle}>
            <div style={{ background: activePill, padding: "10px", borderRadius: "12px" }}>
              <ClipboardCheck size={22} color={activeC} />
            </div>
            <span style={{ fontSize: 11.5, fontWeight: 600, color: labelActive }}>Attend</span>
          </NavLink>
        </div>
      </div>
      {/* --- END SUBMENU WRAPPER --- */}
      ...
      >
            ...
            <>
              ...
              <div
                style={{
                  ...
                  transition: "all .2s",
                  boxShadow: isActive ? `0 0 16px ${activeGlow}` : "none",
                }}
              >
                <tab.icon
                  size={isActive ? 25 : 22}
                  style={{
                    ...
                    transition: "all .2s",
                    filter: isActive ? `drop-shadow(0 0 6px ${activeGlow})` : "none",
                  }}
                />
              </div>
              <span
        ...
        >
          ...
          >
            <Menu size={26} style={{ color: inactC, strokeWidth: 1.7 }} />
          </div>
          <span style={{ fontSize: 12.5, fontWeight: 500, color: labelInact, lineHeight: 1 }}>Menu</span>
        </button>
      </div>
    </nav>
  );
