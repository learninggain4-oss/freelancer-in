...
// face-api loaded dynamically from CDN at runtime to avoid bundling
let faceapi: any = null;
const loadFaceApi = async () => {
  if (faceapi) return faceapi;
  const url = "https://esm.sh/@vladmandic/face-api@1.7.15";
  faceapi = await import(/* @vite-ignore */ url);
  return faceapi;
};
      ...
      <DialogContent className="max-w-none w-screen h-screen p-0 border-none rounded-none bg-slate-950 flex flex-col justify-between overflow-hidden font-sans select-none">
        ...
        <div className="relative w-full h-full flex items-center justify-center bg-zinc-950">
          ...
          {step === "preview" && capturedImage && (
            <>
              <img loading="lazy" decoding="async" src={capturedImage} alt="Identity token" className="w-full h-full object-cover" />
